...
class Action
{
	...
	function save_user()
	{
		...
		if ($chk > 0 && $need != "update") {
			return 2;
			exit;
		}
		if ($type == 0 || empty($type)) {
			$result = $this->db->query("SELECT type FROM users WHERE id = '$id'");
			if ($result->num_rows > 0) {
				$row = $result->fetch_assoc();
				$existingType = $row['type'];
				$data .= ", type = '$existingType' ";
			} else {
				return 'Failed to retrieve existing user type';
			}
		} else {
			$data .= ", type = '$type' ";
		}
	...
	function save_product()
	{
		...
		if (isset($_FILES['image'])) {
			...
			$new_image_name = "product_" . time() . "." . $imageFileType;
			$new_target_file = $target_dir . $new_image_name;

			if (move_uploaded_file($_FILES["image"]["tmp_name"], $new_target_file)) {
				// The image is successfully uploaded to billing/images folder
				// You may save $new_image_name in the database or perform other operations
	...
	function delete_product()
	{
		...
		// Fetch the image filename before deleting the product record
		$imageQuery = $this->db->query("SELECT image FROM products WHERE id = $id");
		...
		if ($delete) {
			...
			if (!empty($imageName) && file_exists("billing/images/$imageName")) {
				unlink("billing/images/$imageName");
			}
			return 1;
		} else {
	...
	function save_order()
	{
		...
		$data .= ", discount = '$discount' "; // Added line for discount
		$data .= ", after_discount = '$after_discount' "; // Added line for after_discount
